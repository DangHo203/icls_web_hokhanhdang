...
import { apiRegister, sendEmail, verifyEmail } from "./regiserService";
...
export default function Register() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            const rs = await sendEmail({ email: values.email });

            if (rs.status === 200) {
                setIsSending(false);
                setIsRegistered(true);
                console.log("Good");
            } else if (rs.status === 410) {
                Swal.fire({
                    icon: "error",
                    title: "Oops...",
    ...
    const handleResend = async () => {
        try {
            await sendEmail({ email: formik.values.email });
        } catch (error) {
            console.log(error);
        }
    };
    ...
    return isRegistered ? (
        <div className="w-screen h-screen flex flex-col grow justify-center items-center px-20 py-40 text-black bg-white max-md:px-5 max-md:py-24 max-md:max-w-full max-sm:mr-6">
            <OTPVerification
        ...
        <div className="w-screen h-screen flex flex-col grow justify-center items-center px-20 py-5 text-black bg-[#F4F7FF] max-md:px-5 max-md:py-24 max-md:max-w-full max-sm:mr-6">
            {isSending && <Loading />}

            <div
                className="wow fadeInUp relative mx-auto max-w-[525px] overflow-hidden rounded-[30px] shadow-form bg-white dark:bg-dark-2 py-5 px-5 text-center sm:px-12 md:px-[60px]"
                data-wow-delay=".15s"
            >
                ...
                <span className="text-[35px] font-bold">Register</span>
                <form onSubmit={formik.handleSubmit}>
                    <div className="mb-[10px]">
                        ...
                        {formik.errors.name && formik.touched.name ? (
                            <div className="w-full text-red-500 text-[12px] flex justify-start">
                                * {formik.errors.name}
                            </div>
                        ) : null}
                    </div>
                ...
                <span className="relative block text-center z-1 mb-2">
                ...
                <ul className="flex justify-between -mx-2 mb-2">
